...
context:'2018~2019년 사이 [BERT](#/p/bert)·[GPT-2](#/p/gpt2)·RoBERTa·XLNet 등 Transformer 계열 사전학습 모델이 쏟아졌지만, 각 논문은 서로 다른 프레임워크(TensorFlow/PyTorch)와 저장소 구조로 자체 코드를 공개했다. 한 모델에서 다른 모델로 옮기려면 토크나이저·체크포인트 포맷·추론 코드를 매번 새로 익혀야 했고, 파인튜닝된 모델을 동료와 공유할 표준 방법도 없었다. Transformers 라이브러리는 이 파편화를 정면으로 겨눈다 — **아키텍처마다 다른 코드를, 하나의 API와 하나의 공유 허브로 통일할 수 있는가?**',
...
legacy:[
 '**Model Hub가 이후 datasets·diffusers·PEFT 등 Hugging Face 생태계 전체의 축**이 되어, NLP를 넘어 비전·음성·멀티모달 모델까지 같은 방식으로 공유됨',
