...
ideas:[
 ...
 {h:'메모리 효율적 추론: 넓은 텐서를 통째로 들고 있지 않는다',
  ...
  d:'모바일 추론에서는 연산량(MAdd)뿐 아니라 **한 시점에 메모리에 올려야 하는 최대 텐서 크기**도 병목이다. 역 잔차 구조는 확장된 넓은 텐서가 병목 안에서만 살아있고 블록 경계에서는 다시 좁아지므로, 컴파일러가 확장-압축 연산을 채널별로 쪼개 스트리밍하면 넓은 중간 텐서를 통째로 메모리에 유지할 필요가 없다. 논문의 Table 3은 16비트 활성값 기준 최대 메모리를 MobileNetV1 1600KB, [ShuffleNet](#/p/shufflenet) 600KB 대비 MobileNetV2가 **400KB**로 가장 작다고 보고한다.'}
],
