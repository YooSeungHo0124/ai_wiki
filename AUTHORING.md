...
`split`의 `from`(분기 시작 상자)은 더 이상 자동으로 강조되지 않는다 — 강조하고
싶으면 `from`이나 `branches[]`의 노드 하나에 직접 `acc:true`를 준다(여러 곳에
주면 안 된다는 규칙은 여기도 동일).

### 선택 필드 (2026-09 렌더러 품질 라운드에서 추가/정리, 전부 하위 호환)
- `flow`/`loop`의 `nodes[].a`: 그 노드에서 다음 노드로 가는 화살표 위에 짧은
  라벨을 붙인다(예: `{t:'선형 투영', a:'768d로'}` → 다음 상자로 가는 화살표에
  "768d로"가 뜬다). 지금까지 문서화되지 않았고 실제로도 쓰인 적이 없는
  필드였다 — 렌더러가 화살표 라벨을 스크린리더에서 숨기던 버그(라벨이 있어도
  장식용 선과 함께 통째로 `aria-hidden`)를 고치면서 같이 정리했다. 짧게(8자
  내외) 쓸 것 — 화살표 자리는 좁다.
- `split`의 `branches[].off:true`: 그 갈래가 실제로는 선택/실행되지 않았음을
  낮은 불투명도의 연결선으로 표시한다(예: top-1 MoE에서 선택되지 않은
  나머지 expert들에 `off:true`). 상자 자체는 그대로 읽히고 연결선만 흐려진다.
...
## H. 자주 걸리는 함정 (실작업에서 나온 것들)

- **아포스트로피가 든 영문 인용은 큰따옴표 문자열로 감싼다.**
  `'...Let\'s think...'` 처럼 작은따옴표 안에서 이스케이프하면 실수하기 쉽다.
  `t:"Let's think step by step"` 이 안전하다.
- **표에서 숫자를 뽑을 때 어느 행·열인지 확인한다.** 같은 표에 비슷하거나
  똑같은 값이 여러 번 나오는 일이 흔하다.
  (실제 사례: mT5 의 XNLI 85.0%는 zero-shot 값이고 같은 표의 translate-train
  은 87.8%다. commonsense-lm 의 WSC-273 63.7%는 14개 모델 앙상블 값인데
  다른 하위 분석과 숫자가 우연히 같다.)
- **브리핑을 그대로 믿지 않는다.** 코디네이터가 준 배경 설명도 원문과
  대조한다. (실제 사례: VQ-VAE-2 가 Sparse Transformer 로 사전분포를 학습한다는
  브리핑이 틀렸다. 원문에 그 단어가 한 번도 없고 실제로는 PixelSnail 이다.)
- **그림이 없는 논문은 그림을 넣지 않는다.** 이론 논의 중심 논문, 시스템
  데모 논문, 표만 있는 데이터셋 논문이 그렇다. 표를 그림으로 넣지 말고
  인용을 늘리거나 `figures` 를 생략한다.
- **받은 PDF 의 제목을 `text.txt` 첫 줄로 확인한다.** 같은 연구팀의 다른
  논문이 섞여 오는 일이 실제로 있었다(AlphaGo ↔ AlphaGo Zero).
