...
ideas:[
 ...
 {h:'CVAE로 행동 시퀀스의 다봉성(스타일 변이)을 흡수',
  ...
  d:'같은 상태에서도 사람마다 다르게 움직이는 시연 데이터를 그냥 회귀로 학습하면 여러 그럴듯한 궤적의 평균이라는 물리적으로 말이 안 되는 행동이 나온다. ACT는 CVAE로 학습한다 — 인코더(BERT류 [Transformer](#/p/transformer))가 행동 시퀀스와 관절 관측을 압축해 스타일 변수 $z$를 만들고, 디코더가 $z$·이미지·관절 위치로 행동 시퀀스를 생성한다. 인코더는 학습에만 쓰고 추론 시 $z$는 사전분포 평균인 0으로 고정한다.'},
...
legacy:[
 ...
 '**ALOHA 하드웨어 계열 확장** — ALOHA 2, [Mobile ALOHA](#/p/mobile-aloha) 등 후속 하드웨어가 이 논문의 leader-follower 원격조작 설계를 그대로 계승',
