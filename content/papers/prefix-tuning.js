...
impact:'"태스크 적응 = 가중치 갱신"이라는 등식을 깨고, **입력 공간에서의 적응**이라는 별도 축을 만들었다. 이후 [prompt tuning](#/p/prompt-tuning)(입력층만, 모델이 커질수록 전체 미세조정에 수렴), P-tuning v2 등 연속 프롬프트 계열이 여기서 갈라져 나왔다. 실무적으로는 하나의 동결된 백본에 태스크별 prefix만 갈아끼우는 서빙 구조를 제시했고, 배치 안에서 서로 다른 태스크의 prefix를 섞어 처리할 수 있다는 점이 [Adapter](#/p/adapter)에는 없는 장점이었다. 다만 곧이어 나온 [LoRA](#/p/lora)가 문맥 창을 잡아먹지 않으면서 최적화도 쉽다는 이유로 실무 표준 자리를 가져갔다.',
